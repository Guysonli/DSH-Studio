...
DeepSeek Harness（dsh）官方形态是命令行启动的本地 Web 服务（`dsh web`，默认端口 3080），用户通过浏览器访问 图形界面。本项目的目标：

1. 将 DSH 打包为**原生桌面应用**（Electron），独立窗口内嵌 图形界面，不依赖浏览器
2. **自动管理 dsh 服务进程**：启动时拉起、退出时清理，用户无感
3. 生成**可分发的 NSIS 安装包**（.exe），目标用户无需安装 Node/命令行，首次运行通过向导配置 API Key
4. **与现有 命令行 安装无缝共存**：共享工作区/会话数据，无冲突
5. **预留官方 dsh 升级通道**：dsh 包自动更新，不落后于官方 网页版

核心原则：**桌面版只是外壳（容器 + 管家），不修改、不裁剪 DSH 本身**。图形界面 功能与官方 网页版 100% 一致。
...
│  └─ 内嵌 DSH 图形界面      http://127.0.0.1:<port>      │
└───────────────────────────────────────────────────┘
```
...
1. 应用启动 → 单实例锁检查 → 端口探测
2. 按状态机推进：初始化（如需）→ 启动/连接服务 → 就绪探测 → 窗口加载 图形界面
...
- **已有服务在跑**（HTTP 可达）→ 不启动新服务，直接内嵌连接现有实例。此时运行时状态（含正在运行的会话）与 命令行 浏览器完全一致
...
启动后（异步，静默）:
  ③ 请求 npm 软件源 比对最新版本号
     ├─ 有新版 → 后台下载 压缩包 → 解压到 自更新副本区 → 下次启动生效
     └─ 失败 → 忽略，继续用现有版本
```

- **坏版本防护**：自更新副本版本启动失败（spawn 报错/服务起不来）→ 自动回退捆绑基线
- **数据保护**：重装/升级外壳只覆盖安装目录，`~/.dsh`（配置、密钥、会话、自更新副本）永不触碰
- 版本信息显示：设置页/关于页展示当前 dsh 版本与可用版本

### 3.6 单实例锁

- Electron `requestSingleInstanceLock` 防桌面版双开
- 与 命令行 并发的极端竞态（同一秒两边同时拉起）：由 port-probe 探测兜底——后启动方探测到端口被占且不可达时提示"端口被占用，请关闭其他实例"，不强行抢端口
...
- API Key 仅写入用户目录 `.dsh/.credentials.yaml`（与官方 命令行 相同位置，格式兼容，可互用）
...
- **手动验收清单**：
  1. 全新环境模拟（临时清空 `~/.dsh`）：向导 → 密钥配置 → 启动 → 图形界面 加载
  2. 密钥已配置：直接启动
  3. 命令行 实例运行中 → 桌面版直接连接（同一会话可见）
  4. 端口 3080 被其他程序占用 → 自动换端口
  5. 退出应用 → 服务进程树被清理（无残留）
  6. 服务崩溃（kill 子进程）→ 错误页 + 重试恢复
  7. 安装包安装/卸载；卸载后 `~/.dsh` 保留
  8. 双开桌面版 → 聚焦已有窗口
  9. 更新管理器：自更新副本存在时优先加载；坏版本回退

## 9. 非目标（YAGNI）

- 不修改 dsh 源码，不新增 网页版没有的功能
- 不做托盘常驻、开机自启（用户未要求）
...
2. 服务管理：端口探测 + 服务拉起 + 就绪加载 图形界面 + 退出清理
3. 首次运行向导 + bootstrap 初始化
4. 更新管理器 + 自更新副本三层加载
5. electron-builder 打包 + 手动验收清单执行

## 11. 设计修订（2026-08-28）：与原生 命令行 共存架构

原方案（第 3.5 节）以"自更新副本三层加载"为核心，v0.1 实现后经实测发现两个问题：自更新副本的依赖补齐（junction 链接）与 dsh 原生的 `healProfilesModuleFallback` 组合会产生自引用环、导致服务无法启动；且自更新副本与用户全局命令行版本易漂移。修订如下：

- **独立性原则**：Studio 只对它自己的 dsh 负责（自更新副本 + 捆绑基线），与其他途径安装的 dsh（npm/npx/Volta）在安装/卸载/版本控制上完全隔离；唯一共享的是 `~/.dsh` 数据层（密钥/会话/工作区全部互通）。
- **dsh 来源（Studio 启动用）**：
  1. 自更新副本 `~/.dsh/vendor/dsh`（Studio 托管更新；要求真实安装——node_modules 为真实目录而非 junction 链接，拒绝"借壳"假阳性）
  2. 捆绑基线（安装包自带，离线兜底）
  3. 均不可用时 → 选择页列出电脑上全部完整 dsh（自带/全局/npx 缓存，均标注版本），由用户挑选，仅本次生效，不修改任何安装
- **完整性校验**：每个候选安装以 Node 真实解析校验核心包（dsh-app-boot/dsh-base/dsh-web-app）可解析，半安装、junction 环、悬空链接一律跳过
- **端口共存策略（透明协调，不硬碰）**：
  - 3080 空闲 → 启动 Studio 自己的实例
  - 3080 已有 dsh 服务 → 首次弹窗询问「建立连接（复用实例，共享运行中会话）」或「启动独立实例（换空闲端口）」，可勾选记住选择（portPolicy 设置）
  - 非 dsh 程序占用 → 自动换空闲端口（decidePort allowConnect=false）
- **自动更新归属**：Studio 仅维护自己的自更新副本；不检测、不修改全局/npx 安装
- **凭据互认**：API Key 读写采用 dsh 原生 v1 布局（`version: 1` + `refs:`，含 flow 风格兼容），与 命令行 / 网页 Models 配置页完全互读，写入保留其他凭据条目
- **入口即服务**：浏览器可直接访问 `http://127.0.0.1:<port>` 使用原生 Web 图形界面，无需单独安装；终端 命令行 为可选的进阶场景
