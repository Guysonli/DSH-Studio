...
dsh 的会话日志是**单写者**模型：**同一会话同一时刻只应有一个实例在写入**。当多个实例（如 CLI 与 Studio 独立实例）并存且同时操作同一个会话时，可能出现并发写入导致会话日志出现重复序号，被判定为损坏（表现为"历史记录无法获取"）。因此：

- **日常使用建议选「建立连接」**（端口策略弹窗的默认推荐项）：所有入口共用同一个实例，从根本上避免并发写入。
- 使用「启动独立实例」时，避免在多个实例中同时打开/操作同一个会话。
- 会话列表在界面上**按工作区（工作目录）分组展示**：跨实例新建的会话**不会实时出现在另一实例**——需要切换/选中同一工作区后刷新页面（Ctrl+R）。两侧创建会话时选择同一工作区即可互相可见（底层数据始终共享，API 返回全部会话）。
- 若已出现 `corrupt session log`，先让相关实例停止写入该会话，再运行修复工具（将删除重复序号行、无损恢复）：

```powershell
node scripts/repair-session.js "$env:USERPROFILE\.dsh\sessions\<会话目录>" --apply   # 先不加 --apply 预览
```
